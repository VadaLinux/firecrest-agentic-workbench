    ...
    try:
        with urllib.request.urlopen(req, timeout=3.0) as response:
            ...
            if response.status >= 300:
                print(
                    f"ERROR: /v1/models returned HTTP {response.status}",
                    file=sys.stderr,
                )
                sys.exit(1)
            try:
            ...
            if "data" not in data or not isinstance(data["data"], list):
                print(
                    "ERROR: /v1/models response JSON is malformed (missing 'data' list).",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            if target_model not in model_ids:
                print(
                    f"ERROR: Model '{target_model}' not found in /v1/models discovery.",
                    file=sys.stderr,
                )
                sys.exit(1)
