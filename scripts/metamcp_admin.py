...
async def run(args) -> int:
    async with Admin(args.endpoint, api_key()) as admin:
...
async def add_firecrest(admin: Admin, args) -> int:
    """Register firecrest-mcp as an HTTP MCP server and put it in the namespace.
    ...
    created = await admin.call(ADMIN_PREFIX + "metamcp_create_mcp_server", {
        "name": "firecrest",
        "description": "FirecREST HPC job submission, status, logs and files",
    ...
    names = sorted(t.name for t in await admin.tools())
    firecrest = [t for t in names if not t.startswith(ADMIN_PREFIX)]
    print("\n=== tools on the endpoint (excluding admin) ===")
    print(pretty(firecrest))
    return 0
