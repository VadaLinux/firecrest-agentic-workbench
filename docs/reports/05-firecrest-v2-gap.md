...
| **Download file (large)** | No separate path; same `/utilities/download` regardless of size | Separate `POST /filesystem/{system}/transfer/download` (`transferDirectives`-shaped body), S3-presigned — **exercised end to end 2026-09-15 under VDLP-43** against the v1 stack's MinIO; see `docs/hot-cache-v2.md` §7 | A v2 client needs a size-based branch v1 never needed, or can defer this until large-file transfer is actually required. Note the response is a *job*, not bytes: the caller gets a presigned `downloadUrl` plus a `transferJob` whose completion it must track |
